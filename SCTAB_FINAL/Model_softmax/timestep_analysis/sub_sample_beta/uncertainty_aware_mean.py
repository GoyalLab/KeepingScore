...
def subsample_sigmoid_betas(T_train, T_eval, beta_min, beta_max, low, high, device):
    """
    Properly subsample betas by re-evaluating the sigmoid schedule
    at the new timestep positions, rather than linear interpolation.
    
    Args:
        T_train: Number of training timesteps
        T_eval: Number of evaluation timesteps (subsampled)
        beta_min, beta_max, low, high: sigmoid schedule parameters
        device: torch device
        
    Returns:
        betas_sub: [T_eval] tensor of beta values
        step_ids: [T_eval] numpy array of original timestep indices
    """
    # Create sigmoid schedule for evaluation timesteps
    # Use the same warping function as training
    t_eval = torch.linspace(low, high, T_eval).to(device)
    
    # Re-evaluate sigmoid schedule at evaluation points
    sigmoid_t = torch.sigmoid(t_eval)
    betas_sub = beta_min + (beta_max - beta_min) * sigmoid_t
    
    # Map evaluation timesteps to training timestep indices
    # for model input (since model expects timesteps in [1, T_train])
    t_train = torch.linspace(low, high, T_train).to(device)
    step_ids = torch.searchsorted(t_train, t_eval).cpu().numpy()
    step_ids = np.clip(step_ids, 1, T_train)
    
    return betas_sub, step_ids
...
def softmax_posterior_latent(
    ...
    betas_sub, step_ids = subsample_sigmoid_betas(
        T_train, T_eval, beta_min, beta_max, low, high, device
    )
