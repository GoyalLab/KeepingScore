...
# =========================
# Softmax Posterior Inference using vanilla_model.py (no autocast / no chunking)
# =========================
import os
import argparse
import logging
...
betas = vanilla_model.sigmoid_schedule(
    T=args.T, beta_min=args.beta_min, beta_max=args.beta_max,
...
model = diffusion_module.model.to(device)
model.eval()
...
# ----------------------------
# Softmax Posterior Function (no autocast, no chunk)
# ----------------------------
@torch.no_grad()
def softmax_posterior_latent(
    ...
    # Precompute forward paths
    z_path = torch.empty(B, n_paths, T + 1, D, device=device)
    z_path[:, :, 0, :] = z0_batch[:, None, :]
    for t in range(T):
        z_path[:, :, t + 1, :] = sqrt_a[t] * z_path[:, :, t, :] + sqrt_1ma[t] * eps[None, :, t, :]

    # Compute log-likelihood for all classes directly
    ll_mean_all = torch.zeros(B, num_classes, device=device)
    for c in range(num_classes):
        y_label = torch.full((B * n_paths,), c, device=device, dtype=torch.long)
        ll_chunk = torch.zeros(B * n_paths, device=device)

        for t in range(T):
            zt = z_path[:, :, t, :].reshape(-1, D)
            zt1 = z_path[:, :, t + 1, :].reshape(-1, D)
        ...
        ll_chunk = ll_chunk.view(B, n_paths)
        ll_mean_all[:, c] = ll_chunk.mean(dim=1)
...
for class_idx, true_label_tensor in enumerate(unique_labels):
    ...
    eps_cache = torch.cat([eps_half, -eps_half], dim=0)
