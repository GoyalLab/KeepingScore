...
# ----------------------------
@torch.no_grad()
def softmax_posterior_latent(z0_obs, den_model, betas, T, n_paths=256, num_classes=10, device="cuda"):
    """
    Vectorized softmax posterior inference.
    Computes MAP class and posterior probabilities over all classes in one batch.
    """
    # --- shape setup ---
    if z0_obs.ndim == 1:
        z0_obs = z0_obs.unsqueeze(0)  # [1, D]
    D = z0_obs.shape[-1]

    # --- precompute diffusion paths (same for all classes) ---
    alphas = 1. - betas
    ...
    z_path[:, 0, :] = z0_obs
    for t in range(T):
        z_path[:, t + 1, :] = sqrt_a[t] * z_path[:, t, :] + sqrt_1ma[t] * eps[:, t, :]

    # --- expand for all classes at once ---
    ...
    # --- reshape, average over MC paths, and normalize ---
    ll = ll.view(num_classes, n_paths)                           # [num_classes, n_paths]
    ll_all = ll.mean(dim=1)                                      # [num_classes]
...
# ----------------------------
# Run Softmax on test set
# ----------------------------
def downsampled_betas(original_betas: torch.Tensor, new_T: int) -> torch.Tensor:
    """
    Downsample betas to `new_T` steps but only within the first `new_T`
    original steps — preserving the original noise scale.
    """
    L = len(original_betas)
    # Only use the first new_T indices from the original schedule
    original_subset = original_betas[:new_T]
    
    # Interpolate back to new_T evenly spaced steps
    idxs = torch.linspace(0, new_T - 1, new_T)
    idxs_floor = torch.floor(idxs).long()
    betas_down = original_subset[idxs_floor]
    return betas_down

betas_small_T = downsampled_betas(betas, args.T)

# ----------------------------
# Run inference
# ----------------------------
print(f"[INFO] Running Softmax on test class {class_idx}...")
...
for class_idx, true_label_tensor in enumerate(unique_labels):
    ...
    # ----------------------------
    # Downsample to 300 if necessary
    # ----------------------------
    if len(z0_obs_all) > sample_size:
        torch.manual_seed(42)
        rand_idx = torch.randperm(len(z0_obs_all))[:sample_size]
        z0_obs_all = z0_obs_all[rand_idx]
        logger.info(f"[INFO] Downsampled from {len(class_idxs)} to {sample_size} samples.")
    else:
    ...
    for i in range(z0_obs_all.shape[0]):
        ...
        _, posterior = softmax_posterior_latent(
            idx=i,
            z0_obs=z0_obs_all[i],
            den_model=model,
            betas=betas,
        ...
        map_label = int(np.argmax(post_np))
    ...
    # ----------------------------
    post_np = np.mean(np.stack(posteriors), axis=0)  # average posterior over 300 samples
    map_label = int(np.argmax(post_np))
...
logger.info("All 164 class inferences completed successfully and saved.")
